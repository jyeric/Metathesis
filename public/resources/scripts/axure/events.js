...
$axure.internal(function($ax) {
  ...
  var _createProxies = function(diagramObject, elementId) {
    ...
    if (createFocus) diagramObject.interactionMap.onFocus = {
      proxy: true,
      description: 'OnFocus'
    };
    if (createLostFocus) diagramObject.interactionMap.onLostFocus = {
      proxy: true,
        ...
        $link.click(function(j) {
          return function() {
            ...
            for (var k = 0; k < currentCase.actions.length; k++) {
              ...
              postMessage('axAction', {
                'name': currentAction.displayName
              });
    ...
    query.each(function(dObj, elementId) {
      ...
      if ($ax.public.fn.IsMenuObject(dObj.type)) {
        if ($element.hasClass('sub_menu')) {
          var tableCellElementId = $ax.getElementIdFromPath([dObj.parentCellId], {
            relativeTo: elementId
          });
  ...
  var _mouseLocation = $ax.mouseLocation = {
    x: 0,
    y: 0
  };
  var _lastmouseLocation = $ax.lastMouseLocation = {
    x: 0,
    y: 0
  };
